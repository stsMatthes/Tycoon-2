...
;;;  Originally a query-replace of cmuscheme.el mode,
;;;  but without all the compilation stuff
...
(define-key tl2-mode-map "\C-c\C-c" 'tl2-do-check)
...
Tl2 source.  
    ...
    tl2-load-file loads a Tl2 source file into the Tl2 process.
      Default is the current buffer or the last source loaded.
    tl2-send-definition-and-go and tl2-send-region-and-go
...
(defvar tl2-default-directory nil
  "Working directory for tl2 process (use current working directory if nil)")
...
\(Type \\[describe-mode] in the process buffer for a list of commands.)"
  ...
  (if (not (comint-check-proc "*tl2*"))
      (let* ((cmdlist (tl2-args-to-list cmd))
	     ...
	     (new-def-dir
	      (or tl2-default-directory
		  default-directory))
	     (default-directory new-def-dir)
	     (buf (apply 'make-comint "tl2" (car cmdlist)
			 nil (cdr cmdlist)))
	     (default-directory old-def-dir))
	(set-buffer buf)
	(inferior-tl2-mode)))
