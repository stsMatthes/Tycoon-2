...
class GraphPanel extends Panel implements Runnable {
	...
	graph.getAppletContext().showDocument(u);
    }
...
public class DAG extends Applet {
	...
	String tycoonRoot = getParameter("tycoonRoot");

	String user = "<unknown teamware user>";
	String workspace = "<unknown teamware workspace>";
	String tycoon = "<unknown tycoon root dir>";
	
	for (StringTokenizer t = new StringTokenizer(tycoonRoot, "/") ;
	     t.hasMoreTokens() ; ) {
	  user = workspace;
	  workspace = tycoon;
	  tycoon = t.nextToken(); 
	...
	panel.workspace = user + "/" + workspace;
